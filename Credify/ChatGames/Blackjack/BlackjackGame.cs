...
/*
2 or more people -> if they timeout during bet stage the other players don't see their hand
Other player outcomes (to players)
...
public class BlackjackGame
{
    ...
    private StateEnums.GameState _gameState;
    ...
    public BlackjackGame(PersistenceManager persistenceManager, CredifyConfiguration credifyConfig)
    {
        ...
        _gameState = StateEnums.GameState.WaitingForPlayers;
    }
    ...
    {
        if (_gameState != StateEnums.GameState.WaitingForPlayers) return;
        if (_players.IsEmpty) return;

        _gameState = StateEnums.GameState.RequestPlayerStakes;
    ...
    private async Task RequestPlayerStakesAsync(CancellationToken token)
    {
        ...
        {
            player.Value.State = StateEnums.PlayerState.Playing;
            var playerFunds = await _persistenceManager.GetClientCredits(player.Key);
            if (playerFunds < 10)
            {
    ...
    {
        if (_gameState is not StateEnums.GameState.RequestPlayerStakes) return;
        _gameState = StateEnums.GameState.DealCards;
    ...
    {
        _gameState = StateEnums.GameState.RequestPlayerDecisions;

        foreach (var player in _activePlayers)
        {
            if (CalculateHandValue(player.Value.Cards) is 21)
            {
                player.Value.State = StateEnums.PlayerState.Stand;
    ...
    {
        if (_gameState is not StateEnums.GameState.RequestPlayerDecisions) return;
        _gameState = StateEnums.GameState.DealerPlays;
        ...
        foreach (var player in _activePlayers)
        {
            ...
            if (playerValue > 21)
            {
                ...
                player.Value.Outcome = StateEnums.GameOutcome.Lose;
            ...
            if (IsBlackjack(player.Value.Cards))
            {
                if (IsBlackjack(_houseHand))
                {
                    ...
                    player.Value.Outcome = StateEnums.GameOutcome.Push;
                ...
                else
                {
                    ...
                    player.Value.Outcome = StateEnums.GameOutcome.Blackjack;
            ...
            if (houseValue > 21)
            {
                ...
                player.Value.Outcome = StateEnums.GameOutcome.Win;
                _players[player.Key].Payout = Convert.ToInt64(Math.Round(_players[player.Key].Stake!.Value *
    ...
    {
        _gameState = StateEnums.GameState.Payout;
        ...
        foreach (var player in _activePlayers)
        {
            ...
            await _persistenceManager.AlterClientCredits(_players[player.Key].Payout!.Value, client: player.Key);
            var playerCredits = await _persistenceManager.GetClientCredits(player.Key);
    ...
    public async Task JoinGameAsync(EFClient player)
    {
        ...
        if (_gameState is not StateEnums.GameState.WaitingForPlayers)
        {
    ...
    public async Task HandleChatAsync(EFClient player, string message)
    {
        ...
        if (playerHand.State != StateEnums.PlayerState.Playing) return;
        ...
        try
        {
            ...
            switch (_gameState)
            {
                case StateEnums.GameState.RequestPlayerStakes:
                    if (_players[player].Stake is not null) return;
                    if (!long.TryParse(message, out var stake)) return;

                    var amount = await _persistenceManager.GetClientCredits(player);
                    if (amount >= stake)
                    {
                        _players[player].Stake = stake;
                        await _persistenceManager.AlterClientCredits(-stake, client: player);
                    ...
                    break;
                case StateEnums.GameState.RequestPlayerDecisions:
                    ...
                    if (!Enum.TryParse<StateEnums.PlayerChoice>(newMessage, true, out var playerDecision)) return;

                    switch (playerDecision)
                    {
                        case StateEnums.PlayerChoice.Hit:
                            ...
                            if (IsBust(player))
                            {
                                playerHand.State = StateEnums.PlayerState.Busted;
                                player.Tell($"{_credifyConfig.Translations.BlackjackTitleShort} " +
                            ...
                            break;
                        case StateEnums.PlayerChoice.Stand:
                            playerHand.State = StateEnums.PlayerState.Stand;
                            player.Tell($"{_credifyConfig.Translations.BlackjackTitleShort} " +
                                        $"{_credifyConfig.Translations.BlackjackPlayerStand.FormatExt(CalculateHandValue(playerHand.Cards))}");
                            break;
                        case StateEnums.PlayerChoice.Cards:
                            await player.TellAsync(new[]
                            {
                                $"{_credifyConfig.Translations.BlackjackTitleShort} {_credifyConfig.Translations.BlackjackDealerInitialCard.FormatExt(_houseHand[0])}",
    ...
    {
        string FormatOutcome(StateEnums.GameOutcome outcome) =>
            outcome switch
            {
                StateEnums.GameOutcome.Blackjack => _credifyConfig.Translations.BlackjackOutcomeBlackjack,
                StateEnums.GameOutcome.Win => _credifyConfig.Translations.BlackjackOutcomeWin,
                StateEnums.GameOutcome.Lose => _credifyConfig.Translations.BlackjackOutcomeLose,
                StateEnums.GameOutcome.Push => _credifyConfig.Translations.BlackjackOutcomePush,
                _ => string.Empty
            };
    ...
    private List<EFClient> GetDecisionStateRemainders()
    {
        var players = _activePlayers
            .Where(x => x.Value.State is StateEnums.PlayerState.Playing)
            .Select(x => x.Key)
            .ToList();
        return players;
    }
    ...
    private int CalculateHandValue(IEnumerable<BlackjackCard> hand)
    {
        var totalValue = 0;
        var aces = 0;
    ...
    private bool IsBlackjack(IEnumerable<BlackjackCard> hand)
    {
